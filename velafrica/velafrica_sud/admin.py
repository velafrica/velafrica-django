...
class ContainerAdmin(ImportExportMixin, DjangoObjectActions, SimpleHistoryAdmin):
    ...
    list_display = ['pickup_date', 'container_no', 'organisation_from', 'warehouse_from', 'partner_to', 'velos_loaded',
                    'velos_unloaded', 'spare_parts', 'booked', 'notes']
    ...
    fieldsets = (
        (None, {
            'fields': (
                'container_no', 'warehouse_from', 'organisation_from', 'partner_to', 'velos_loaded', 'velos_unloaded',
                'spare_parts', 'stocklist', 'booked')
        }),
        ('Transport', {
        ...
        if obj:
            ...
            if result:
                self.message_user(request,
                                  u"Container {} erfolgreich verbucht. {} von {} Trackings aktualisiert. ".format(
                                      obj.id, result[2], result[1]))
            else:
...
class PartnerSudAdmin(SimpleHistoryAdmin):
    ...
    readonly_fields = ['get_googlemaps_link', 'get_name', 'get_website', 'get_description', 'get_country',
                       'get_facebook', 'get_contact']
    inlines = [ContainerInline, PartnerStaffInline]
    fieldsets = (
        (None, {
            'fields': (
                'organisation', 'get_name', 'get_contact', 'get_website', 'get_facebook', 'get_description', 'image')
        }),
        ('Location', {
...
class ReportAdmin(ImportExportMixin, SimpleHistoryAdmin):
    ...
    readonly_fields = ['economic_bicycles_turnover_USD', 'economic_turnover_total_USD',
                       'economic_spareparts_turnover_USD', 'economic_services_turnover_USD',
